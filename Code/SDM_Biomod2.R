...
myResp <- OCC_ENV_2017to2020$SPECCODE

# XY coordinates
myXY <- OCC_ENV_2017to2020[ , c("Long","Lat") ]
...
myExpl  <- OCC_ENV_2017to2020[ , env.var ]
...
myData.all <- BIOMOD_FormatingData( resp.var      = myResp,
                                    ...
                                    eval.resp.var = OCC_ENV_2020_SL[, "SPECCODE" ],
                                    eval.expl.var = OCC_ENV_2020_SL[, env.var ],
                                    eval.resp.xy  = OCC_ENV_2020_SL[, c("Long","Lat") ] )
...
myBiomodModel.all_v1.0 <- BIOMOD_Modeling(
  ...
  modeling.id       = as.character("SDM_2017to2020_Global_eval2020_GAM"), #a character corresponding to the name (ID) of the simulation set (a random number by default)
...
myBiomodModel.opt_v2.0 <- BIOMOD_Modeling(
  ...
  bm.options        = myBiomodOption,
  modeling.id       = as.character("SDM_2017to2020_evalOpt2020_GAM"), #a character corresponding to the name (ID) of the simulation set (a random number by default)
  models            = c("GAM"), #, "CTA", "GAM", "RF"),           #a vector containing model names to be computed, must be among GLM, GBM, GAM, CTA, ANN, SRE, FDA, MARS, RF, MAXENT.Phillips, MAXENT.Phillips.2
  models.pa         = NULL,                                       #a list containing for each model a vector defining which pseudo-absence datasets are to be used, must be among colnames(bm.format@PA.table)
  CV.strategy       = "random",                                   #a character corresponding to the cross-validation selection strategy, must be among random, kfold, block, strat, env or user.defined
  CV.nb.rep         = 5,                                          #if strategy = 'random' or strategy = 'kfold', an integer corresponding to the number of repetitions to be done for calibration/validation splitting
...
g.data          <- rbind( g.1$tab, g.2$tab, g.3$tab )
g.data[,"name"] <- c( "Calib.",
                      "Valid.",
                      "Eval."  )
g.data[,"low1"] <- g.data[,"mean1"] - g.data[,"sd1"]
g.data[,"up1"]  <- g.data[,"mean1"] + g.data[,"sd1"]
g.data[,"low2"] <- g.data[,"mean2"] - g.data[,"sd2"]
g.data[,"up2"]  <- g.data[,"mean2"] + g.data[,"sd2"]

# ggplot2 graphic
g.eval <- ggplot( data = g.data,
                  aes( x      = mean1,
                       y      = mean2,
                       colour = name) )   +
...
myBiomodEM <- BIOMOD_EnsembleModeling(
  ...
  models.chosen = "GAM",  #a vector containing model names to be kept, must be either all or a sub-selection of model names that can be obtained with the get_built_models function
  ...
  metric.select.thresh = c(0.7, 0.45), #A vector of numeric values corresponding to the minimum scores (one for each metric.select) below which single models will be excluded from the ensemble model building
  ...
  nb.cpu = 30, #An integer value corresponding to the number of computing resources to be used to parallelize the single models computation
  seed.val = 5, #An integer value corresponding to the new seed value to be set
  do.progress = TRUE #A logical value defining whether the progress bar is to be rendered or not
)
